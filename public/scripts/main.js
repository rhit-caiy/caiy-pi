...
rhit.max=rhit.pi.length;// 5000 digits
...
rhit.SelectPageController=class{
	constructor(){
		document.querySelector("#start").addEventListener("click",(event)=>{
			...
			if(this.startIndex!=null&&this.startIndex>0&&this.startIndex<rhit.max){//may add more
	...
	constructor(){
		...
		document.querySelector("#digitIndexButton").addEventListener("click",()=>{
			...
			if(index.includes("-")){
				...
				document.querySelector("#digitText").innerHTML=rhit.pi.charAt(index+1);
			}
		});
	}
}
...
rhit.MethodTypeController=class{
	constructor(){
		...
		if(urlParams.get("id")){
			// var position=document.querySelector(urlParams.get("id")).getBoundingClientRect();
			// window.scrollTo(1000,1000);
			$('body').scrollspy({ target: `#${urlParams.get("id")}` })
		}
	}
	_createCard(mt){
		console.log(mt);
